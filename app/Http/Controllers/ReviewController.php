...
class ReviewController extends Controller
{
    ...
    public function store(StoreReviewRequest $request, int $apartment_id) {
        ...
        $ableToRate = Booking::where('user_id',$user_id)->where('apartment_id',$apartment_id)->first();
        if ($ableToRate) {
            $bookingIsExpired = Carbon::parse($ableToRate->end_date)->lt(Carbon::today());
            if ($bookingIsExpired) {
            ...
            return response()->json(['you can\'t rate this apartment because your booking didn\'t expired']);
        }
        return response()->json(['you can\'t rate this apartment because you didn\'t booked it']);
    }
